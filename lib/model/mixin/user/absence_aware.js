...
var
    ...
    // Accounting year lib
  AccountingYear = require('../../accounting_year');
...
module.exports = function(sequelize){
  ...
  this.promise_my_leaves = function(args){
    ...
    var accounting_year = new AccountingYear({year: year});
    ...
    where_clause['$or'] = {
      date_start : {
        $between : [
          accounting_year.start_at().format('YYYY-MM-DD'),
          accounting_year.end_at().format('YYYY-MM-DD'),
        ]
      },
      date_end : {
        $between : [
  ...
  this.get_automatic_adjustment = function(args) {
    ...
    var now             = (args && args.now) ? moment(args.now) : moment();
    var accounting_year = new AccountingYear({year: now});

    // if (
    //   moment(this.start_date) < accounting_year.start_at()
    //   && ( ! this.end_date || moment(this.end_date) > accounting_year.end_at() )
    // ){
    //     return 0;
    // }

    if (moment(this.start_date) > accounting_year.start_at()) {
        var start_date = moment(this.start_date);
    } else {
        var start_date = accounting_year.start_at();
    }

    // if (this.end_date && moment(this.end_date) < accounting_year.end_at()) {
    //     var end_date = moment(this.end_date);
    // } else {
    //     var end_date = accounting_year.end_at();
    // }
