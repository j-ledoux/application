...
get_deducted_days : function(args) {
  ...
  if (args && args.hasOwnProperty('year')) {
    ...
    accounting_year = new AccountingYear({year: year});
  } else {
  ...
  leave_days = _.filter(
    _.map(this.get_days(), function(leave_day){
      ...
      // We check that this leave is in current accounting year
      if (moment(leave_day.date) < accounting_year.start_at() || moment(leave_day.date) > accounting_year.end_at()) return;
