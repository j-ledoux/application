...
var
  ...
  remoteUrl      = config.get('scheme') + config.get('domain');
...
module.exports = function(passport) {
  ...
  router.get('/login', function(req, res){
      res.render('login', {
          ...
          url_to_the_site_root : req.get('host').indexOf('app.timeoff') < 0 ? '/' : remoteUrl,
      });
  });
  ...
  router.post('/register', function(req, res){
      ...
      // be eliminated
      if (!validator.matches(company_name, /^[a-z0-9\u00C0-\u017F \.\,\-']+$/i)){
