...
var express   = require('express'),
    ...
    // Accounting year lib
    AccountingYear = require('../model/accounting_year');
...
router.get('/', function(req, res) {
    ...
    var accounting_year = new AccountingYear();
...
function get_and_validate_user_parameters(args) {
    ...
    if (!validator.matches(name, /^[a-z0-9\u00C0-\u017F \.\,\-']+$/i)){
        req.session.flash_error(
            'New name of '+item_name+' should contain only letters and numbers'
        );
    }

    if (!validator.matches(lastname, /^[a-z0-9\u00C0-\u017F \.\,\-']+$/i)){
        req.session.flash_error(
    ...
    // Fix adjustement empty value issue
    if (adjustment == '') {
        adjustment = 0;
    } else if (adjustment && ! validator.isFloat(adjustment) ) {
      req.session.flash_error(
